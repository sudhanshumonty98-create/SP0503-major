...
if os.path.exists(ENCODINGS_FILE):
    ...
    print(f"Loaded {len(encodeListKnown)} encodings for {len(set(classNames))} persons")
else:
    print("Computing encodings (first run)...")
    myList = [f for f in os.listdir(path) if os.path.isdir(os.path.join(path, f))]
    
    # for a person with multiple image of them in one folder
